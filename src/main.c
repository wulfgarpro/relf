...
int main(int argc, char *argv[]) {
  ...
  Relf_Cli_Args args = {0};
  Relf_Cli_Status status =
      relf_cli_parse_args(&args, argc, (const char **)argv);
  switch (status) {

  case RELF_CLI_ERR: {
    relf_cli_print_usage(&args);
    return EXIT_FAILURE;
  }
  case RELF_CLI_HELP: {
  ...
  default: {
    ...
    case RELF_E_CLASS_64:
      // Read ELF Header
      nbytes =
          fread(&ehdr.ehdr.elf64_ehdr, 1, sizeof(ehdr.ehdr.elf64_ehdr), fd);
      ...
      phdrtab.phnum = phnum;
      phdrtab.phdrs.elf64_phdr =
          calloc(phnum, sizeof(*phdrtab.phdrs.elf64_phdr));
      ...
      shdrtab.shnum = shnum;
      shdrtab.shdrs.elf64_shdr =
          calloc(shnum, sizeof(*shdrtab.shdrs.elf64_shdr));
      if (!shdrtab.shdrs.elf64_shdr) {
        fprintf(stderr, "ERROR: allocating memory for section header table\n");
        goto cleanup;
      }

      nitems = fread(shdrtab.shdrs.elf64_shdr,
                     sizeof(*shdrtab.shdrs.elf64_shdr), shnum, fd);
      if (nitems != shnum) {
