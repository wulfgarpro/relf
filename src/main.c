...
int main(int argc, char *argv[]) {
  ...
  bool print_ehdr = false;
  bool print_phdr = false;
  bool print_shdr = false;

  Relf_Cli_Status status = relf_cli_parse_args(&args, argc, argv);

  if (status == RELF_CLI_ERR) {
  ...
  if (args.flags & RELF_CLI_PRINT_EHDR)
    print_ehdr = true;
  if (args.flags & RELF_CLI_PRINT_PHDR)
    print_phdr = true;
  if (args.flags & RELF_CLI_PRINT_SHDR)
    print_shdr = true;
